...
def ask_question(
    ...
    try:
        ...
        if GREETING_PATTERN.fullmatch(question):
            ...
            history = [
                # Cap each turn at 400 chars — long assistant answers (like lists)
                # would burn Groq tokens if included in full in history.
                {"role": message.role, "content": message.content[:400]}
