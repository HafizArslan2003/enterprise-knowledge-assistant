    ...
    <div className="flex-1 flex flex-col h-full relative z-20 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-6">
        {messages.map((msg) => (
          ...
          >
            {msg.sender === 'assistant' && (
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-agilo-primary to-agilo-bright flex items-center justify-center text-white shrink-0 shadow-lg shadow-agilo-primary/20">
            ...
            <div className={`space-y-3 max-w-2xl ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-agilo-bg border border-agilo-border text-xs font-semibold text-agilo-navy shadow-sm">
                  {msg.usedDocumentSearch ? (
                    <>
                      ...
                      <span>🔎 Document Search Used</span>
                    </>
                  ) : (
                    <>
                      <Brain className="w-3.5 h-3.5 text-agilo-deep" />
                      <span>🧠 Answered Directly</span>
                    </>
                  )}
                </div>
              )}
              ...
              <div
                className={`p-5 rounded-3xl text-sm leading-relaxed ${
                  msg.sender === 'user'
                    ? 'bg-agilo-primary text-white shadow-lg shadow-agilo-primary/20 rounded-tr-sm'
                    : 'glass-card text-agilo-navy rounded-tl-sm border border-agilo-border'
                }`}
              >
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-agilo-border/60">
                    <span className="text-[11px] font-bold text-agilo-navy uppercase tracking-wider block mb-2">
                      Cited Enterprise Sources ({msg.sources.length})
                    </span>
                    <div className="flex flex-wrap gap-2">
                      {msg.sources.map((src) => (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-white border border-agilo-border hover:border-agilo-bright text-xs font-medium text-agilo-navy hover:text-agilo-primary transition-all flex items-center gap-2 shadow-sm hover:shadow"
                        >
              ...
              {/* Timestamp */}
              <div
                className={`text-[10px] text-agilo-secondary px-1 ${
            ...
            {msg.sender === 'user' && (
              <div className="w-10 h-10 rounded-2xl bg-agilo-navy flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-md">
                HI
              </div>
            )}
          </motion.div>
        ...
        {isGenerating && (
          ...
          >
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-agilo-primary to-agilo-bright flex items-center justify-center text-white shrink-0 shadow-lg shadow-agilo-primary/20 animate-pulse">
              <Sparkles className="w-5 h-5" />
            </div>
      ...
      <div className="p-4 lg:p-6 max-w-4xl mx-auto w-full">
        <form onSubmit={handleSend} className="relative">
          <div className="glass-panel rounded-3xl p-3 shadow-2xl border border-agilo-border/80 focus-within:border-agilo-bright focus-within:ring-2 focus-within:ring-agilo-bright/20 transition-all">
            <textarea
              value={inputText}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-agilo-border/40">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="p-2 rounded-xl hover:bg-agilo-bg text-agilo-secondary hover:text-agilo-primary transition-colors"
                  title="Attach File"
                >
              ...
              <button
                ...
                className="py-2 px-5 rounded-xl bg-gradient-to-r from-agilo-deep to-agilo-primary text-white text-xs font-semibold shadow-md hover:shadow-lg hover:shadow-agilo-primary/30 transition-all disabled:opacity-40 flex items-center gap-2"
              >
                <span>Send</span>
